function [t,s,values] = twoBodyProblemPerturbed(mu,Rp,s0,nPeriods,tspan,perturbation)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% twoBodyProblemPerturbed: function created to solve the two bod problem 
%                          for the perturbed case by numerical integration
% DESCRIPTION:
%   Solves the Two-Body Problem via numerical integration ALSO IN THE 
%   PERTURBED CASE.
%   The function propagates the initial state vector [r; v] forward in time
%   using the ode solver (ode113) .
%   Additionally, it generates a 3D plot of the computed trajectory.
%
%
% INPUTS
...
%   perturbation:   type of perturbation considered
%
% OUTPUT
...
if isempty(perturbation)
    disp('ERROR: no perturbations added')
end
...
if strcmp(type, 'closed')
    ...
    if isempty(nPeriods)
        nPeriods = tspan(end)/T;
        % tspan = linspace(0 , t , 1e5);
    elseif isempty(time)
...
if strcmp(perturbation, 'J2')
    ...
    aJ2 = @(rvec) (3/2)*J2*mu*(Rp^2)/norm(rvec)^5 * ...
                [ rvec(1)*(5*rvec(3)^2/norm(rvec)^2 - 1);
                  rvec(2)*(5*rvec(3)^2/norm(rvec)^2 - 1);
                  rvec(3)*(5*rvec(3)^2/norm(rvec)^2 - 3) ];

    % define state derivatives
    twoBodyPert = @(t,s) [ s(4:6);
                     -mu * s(1:3)/norm(s(1:3))^3 + aJ2(s(1:3)) ];

    
% elseif
% **********here i can add other perturbations************
...
[~,s_bp] = ode113(twoBody, tspan, s0, options);
[t,s] = ode113(twoBodyPert, tspan, s0, options);
...
y = s(:,2);
z = s(:,3);
C = t / T;  % set color as function of number of periods
pertPlot(x,y,z,C,nPeriods)

% plot unperturbed orbit
x_bp = s_bp(:,1);
y_bp = s_bp(:,2);
z_bp = s_bp(:,3);
plot3(x_bp,y_bp,z_bp,'r','LineWidth',2)
...
legend('tbpPertOrbit' , 'tbpOrbit', 'Location', 'southeast')
